...
- (void)viewDidLoad {
    ...
    [self.homeCityTF setReturnKeyType:UIReturnKeyDone];
...
- (IBAction)continueTouchUp:(UIButton *)sender {
    [[NSUserDefaults standardUserDefaults] setObject:self.homeCityTF.text forKey:HOME_CITY];
    
    [[[UserInfo sharedInstance] userData] setValue:self.homeCityTF.text forKey:HOME_CITY];
...
- (IBAction)skipTouchUp:(UIButton *)sender {
    ...
    [[[UserInfo sharedInstance] userData] setValue:EMPTY_STRING forKey:HOME_CITY];
...
- (void)showScreen4 {
    Screen6ViewController *screen4VC = [self.storyboard instantiateViewControllerWithIdentifier:@"Screen6ViewController"];
...
- (void)setFontSizeForControls {
    ...
    [self.homeCityTF setFont:[UIFont systemFontOfSize:size2]];
