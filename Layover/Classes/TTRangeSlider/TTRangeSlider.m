...
- (void)layoutSubviews {
    ...
    self.sliderLine.frame = CGRectMake(lineLeftSide.x, lineLeftSide.y, lineRightSide.x-lineLeftSide.x, self.lineHeight * 2);
...
//    self.maxLabel.string = [formatter stringFromNumber:@(self.selectedMaximum)];
    if (self.selectedMinimum == 61.0) {
        [self.minLabel setString:@"60+"];
    }else{
        [self.minLabel setString:[NSString stringWithFormat:@"%ld", (long)self.selectedMinimum]];
    }
    
    if (self.selectedMaximum == 61.0) {
        [self.maxLabel setString:@"60+"];
    }else{
        [self.maxLabel setString:[NSString stringWithFormat:@"%ld", (long)self.selectedMaximum ]];
    }
...
-(void)setMinLabelText:(NSString *)text{
    _minLabelText = text;
    [self.minLabel setString:text];
}

-(void)setMaxLabelText:(NSString *)text{
    _maxLabelText = text;
    [self.maxLabel setString:text];
}
